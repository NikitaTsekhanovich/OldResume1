...
class Crazy_tank(Sprite):
    ...
    def __init__(self, ai_settings, screen, bullets_tank, save_data):
        ...
        images = [
            sprites.subsurface(32 * 4, 0, 13 * 2, 15 * 2),
            sprites.subsurface(32 * 4, 16 * 2, 13 * 2, 15 * 2)
        ]
    ...
    def move_next(self, blocks):
        ...
        gf.fire_bullet(self.ai_settings, self.screen,
                       None, None, False, False, None, None, None, None, False,
                       ((self.x,
                         self.y),
                        (self.moving_look_up,
    ...
    def move_up(self, blocks):
        if self.moving_up:
            if self.rect.y != 0 and self.touch_tile_up(blocks):
                ...
                self.moving_look_left = False
                self.moving_look_down = False
                self.y -= self.ai_settings.crazy_tank_speed
                self.rect.y = self.y
            else:
                self.moving_up = False
    ...
    def move_left(self, blocks):
        if self.moving_left:
            if self.rect.x != 0 and self.touch_tile_left(blocks):
                ...
                self.moving_look_up = False
                self.moving_look_down = False
                self.x -= self.ai_settings.crazy_tank_speed
                self.rect.x = self.x
            else:
                self.moving_left = False
                if int(random.uniform(0, 2)) == 0:
